...
def makeProductCollection(colOptions,netOptions,variable,mapBounds,startDate,endDate,maxCloudcover,inputScaleSize) :
    ...
    # make s2_cloudless (For Sentinel-2)
    ...
    if (input_collection.size().getInfo() > 0):
        ...
        if variable == "Surface_Reflectance":
            products = input_collection.combine(s2_cloudless_col)
        else:
            ...
            products =  products.combine(estimateSL2P).combine(uncertaintySL2P.select("error"+variable)).combine(s2_cloudless_col)
...
def samplestoDF(sampleFeature):
    ...
    return sampleDF
def sampleTimeSeries(siteList,imageCollectionName,algorithm,variableName='LAI',maxCloudcover=100,outputScaleSize=30,inputScaleSize=30,bufferSpatialSize=0,bufferTemporalSize=[0,0],subsamplingFraction=1,outputFileName=None,feature_range=[0,np.nan]):
    ...
    for input in siteList:  
        ofn='_'.join([os.path.split(os.path.abspath(input))[-1],imageCollectionName.replace('/','_'),variableName,nlcd,algorithm.__name__+'.pkl'])
        ...
        for n in range(sampleRecords.size().getInfo()) :
            result = []
            site = ee.Feature(sampleRecords.get(n))
            if site.get('nlcd').getInfo()==nlcd:
