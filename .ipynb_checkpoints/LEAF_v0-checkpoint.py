...
def makeProductCollection(colOptions,netOptions,variable,mapBounds,startDate,endDate,maxCloudcover,inputScaleSize) :
    ...
    if (input_collection.size().getInfo() > 0):
        print('%d images found.'%(input_collection.size().getInfo()))
        image = input_collection.first()
        ...
        print('Reflectance data: Table of %d rows| columnss:%s'%(input_collection.size().getInfo(),input_collection.first().bandNames().getInfo()))
        if variable == "Surface_Reflectance":
            ...
            image = input_collection.first() 
            print([image.getInfo()['bands'][ii]['id'] for ii in range(len(image.getInfo()['bands']))])
    ...
    print('Ouput: Table of %d rows| columnss:%s'%(products.size().getInfo(),products.first().bandNames().getInfo()))
    return products
...
def sampleSites(siteList,imageCollectionName,algorithm,variableName='LAI',maxCloudcover=100,outputScaleSize=30,inputScaleSize=30,bufferSpatialSize=0,bufferTemporalSize=[0,0],subsamplingFraction=1,outputFileName=None):   
    ...
    for input in siteList: 
        ...
        pas=500
        for ss in range(0,sampleRecords.size().getInfo(),pas):
            ss_f=np.min([ss+pas,sampleRecords.size().getInfo()])
            print(ss,ss_f)  
            result = []
            for n in range(ss,ss_f) :       
                if not outputFileName:
                    outputFileName='_'.join([os.getcwd()+os.path.split(os.path.abspath(input))[-1],str(ss),datetime.now().strftime("%Y_%m_%d_%Hh_%mmn_%Ss")+'.pkl'])
                print(outputFileName)    
                # select feature to process
                site = ee.Feature(sampleRecords.get(n))
                #print(site.getInfo()['geometry'])
                #site_df=pd.DataFrame.from_dict(site.getInfo()['properties'], orient='index')
                #print(site_df)
                ...
                if ( defaultDate==False ):
                    ...
                    if ("system:time_end" in site.propertyNames().getInfo()):
                        ...
                        endDate = datetime.fromtimestamp(ee.Date(site.get('system:time_start')).getInfo()['value']/1000) + timedelta(days=bufferTemporalSize[1])  
                    endDatePlusOne = endDate + timedelta(days=1) 
                ...
                for index, Dates in dateRange.iterrows():
                    ...
                    if sampleFeature :
                        #print(pd.DataFrame.from_dict(sampleFeature.getInfo(),orient='index'))
                        samplesDF = pd.concat([samplesDF,samplestoDF(sampleFeature)],ignore_index=True)
                        
                result.append({'feature': ee.Dictionary(ee.Feature(sampleRecords.get(n)).toDictionary()).getInfo(),algorithm.__name__ : samplesDF })
...
def imageSites(siteList,imageCollectionName,algorithm,variableName='LAI',maxCloudcover=0,outputScaleSize=0,inputScaleSize=30,bufferSpatialSize=0,bufferTemporalSize=[0,0],subsamplingFraction=1):  
    ...
    outputDictionary = {}
    collectionOptions = (dictionariesSL2P.make_collection_options(algorithm))
    print('collectionOptions.keys()')
    print(collectionOptions.keys())
    networkOptions= dictionariesSL2P.make_net_options()
    print('networkOptions.keys()')
    print(networkOptions.keys())
    ...
    for input in siteList:        
        ...
        for n in range(0,sampleRecords.size().getInfo()) : 
            ...
            #print(startDate,endDate,endDatePlusOne)
            if ( defaultDate==False ):
                startDate = datetime.fromtimestamp(ee.Date(site.get('system:time_start')).getInfo()['value']/1000) + timedelta(days=bufferTemporalSize[0])
                if ("system:time_end" in site.propertyNames().getInfo()):
                    endDate = datetime.fromtimestamp(ee.Date(site.get('system:time_end')).getInfo()['value']/1000) + timedelta(days=bufferTemporalSize[1])
            ...
            for index, Dates in dateRange.iterrows():
                monthlyCollection = getCollection(site,variableName,collectionOptions[imageCollectionName],networkOptions[variableName][imageCollectionName],maxCloudcover,bufferSpatialSize,inputScaleSize, \
                            outputScaleSize,Dates['startDate'],Dates['endDate'],subsamplingFraction)
